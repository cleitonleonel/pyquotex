...
class TestDeepHistory(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_deep_history_continuity(self) -> None:
        ...
        asset = "EURUSD"
        period = 60
        # Fetch 30 minutes of history
        amount_seconds = 1800

        candles = await self.client.get_historical_candles(asset, amount_seconds, period)

        self.assertGreater(len(candles), 0, "No candles were fetched")
    ...
    async def test_deep_history_amount(self) -> None:
        ...
        asset = "EURUSD"
        period = 60
        # 300 candles = 18000 seconds
        amount_seconds = 18000

        candles = await self.client.get_historical_candles(asset, amount_seconds, period)

        # 300 candles * 60s = 18000s. We should have at least 300 candles.
        # Note: broker might return slightly more or less depending on exact boundaries.
        self.assertGreaterEqual(len(candles), 300, f"Fetched only {len(candles)} candles, expected at least 300")
