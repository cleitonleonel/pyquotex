...
async def ejemplo_completo():
    # Inicialización
    cliente = Quotex(email="tu_correo@gmail.com", password="tu_contraseña", lang="es")
...
## Gestión de Estado (Enums)

PyQuotex utiliza `IntEnum` para un seguimiento de estado robusto. Esto reemplaza las antiguas flags enteras/booleanas
por estados claros y descriptivos.

### Estado de WebSocket

Disponible en `client.api.state.status`:
...
Disponible en `client.api.state.auth_status`:
