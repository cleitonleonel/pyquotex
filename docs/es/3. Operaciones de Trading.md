...
async def buy_and_check_win():
    ...
    status, buy_info = await client.buy(amount, asset, direction, duration)
    if status:
        # Esperar el resultado (El temporizador interno está sincronizado con el servidor Quotex)
        # check_win ahora devuelve una tupla: (win_status, profit)
        win_status, profit = await client.check_win(buy_info["id"])
        
        if win_status == "win":
            print(f"¡Ganó! Lucro: {profit}")
        else:
            print(f"Perdió. Resultado: {profit}")
```
...
async def buy_pending():
    ...
    status, buy_info = await client.open_pending(amount, asset, direction, duration, open_time)
```
...
### 6.2 Verificación en tiempo real (Event-Driven)
```python
async def check_win(id_number):
    # Devuelve una tupla (win_status, profit)
    # win_status: "win" o "loss"
    # profit: Monto del lucro/pérdida
