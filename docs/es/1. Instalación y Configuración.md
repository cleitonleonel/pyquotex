# Instalación y Configuración de PyQuotex

## Tabla de Contenidos
- [Requisitos del Sistema](#requisitos-del-sistema)
- [Instalación](#instalación)
  - [Desde pip](#desde-pip)
  - [Desde GitHub](#desde-github)
- [Configuración Inicial](#configuración-inicial)
- [Manejo de Credenciales](#manejo-de-credenciales)
- [Configuración de SSL/TLS](#configuración-de-ssltls)

## Requisitos del Sistema

Para utilizar PyQuotex, necesitarás:

- Python 3.12 o superior
- OpenSSL en su versión más reciente (con soporte para TLS 1.3)
- Sistema operativo compatible:
  - Linux
  - Windows
  - macOS

### Dependencias Principales

Dependencias principales del entorno de ejecución (se instalan automáticamente):

```
websockets>=12.0
httpx>=0.27.0
beautifulsoup4>=4.12.3
pyfiglet>=1.0.2
fake-useragent==2.2.0
certifi
rich
```

> **Nota:** `numpy` se eliminó en la **v1.4.0** — los indicadores técnicos
> se reescribieron en Python puro (`statistics` de la biblioteca estándar),
> lo que ahorra ~20 MB y desbloquea la instalación en Termux e imágenes
> ligeras. `orjson` tampoco es una dependencia principal; se distribuye
> como el extra opcional `[fast]`. Consulta la tabla de extras opcionales
> en el README del proyecto para `fast`, `socks`, `stealth` y `webapi`.

## Instalación

### Desde pip
Puedes instalar PyQuotex directamente desde GitHub usando pip:

```bash
pip install git+https://github.com/iahmedani/pyquotex.git
```

### Desde GitHub y Poetry
También puedes clonar el repositorio y realizar una instalación local:

- Primero [Instalar Poetry](https://python-poetry.org/docs/#installing-with-the-official-installer)

```bash
git clone https://github.com/iahmedani/pyquotex.git
cd pyquotex
poetry install
```

## Configuración Inicial

Para comenzar a usar PyQuotex, primero debes importar y configurar el cliente:

```python
from pyquotex.stable_api import Quotex

client = Quotex(
    email="tu_email@gmail.com",
    password="tu_contraseña",
    lang="es"  # Idioma por defecto: "pt" (portugués)
)

# Habilitar modo debug (opcional)
client.debug_ws_enable = True
```

### Parámetros de Inicialización

| Parámetro | Tipo | Valor por defecto | Descripción |
|-----------|------|-------------------|-------------|
| `email` | str | None | Email de la cuenta Quotex |
| `password` | str | None | Contraseña de la cuenta |
| `host` | str | "qxbroker.com" | Host de la plataforma |
| `lang` | str | "pt" | Idioma (pt, en, es) |
| `user_agent` | str | "Quotex/1.0" | User-Agent para solicitudes |
| `root_path` | str | "." | Directorio raíz |
| `user_data_dir` | str | "browser" | Directorio de datos del navegador |
| `asset_default` | str | "EURUSD" | Activo por defecto |
| `period_default` | int | 60 | Período por defecto en segundos |
| `proxies` | dict | None | Configuración de proxy |

## Manejo de Credenciales

Hay dos formas principales de manejar las credenciales:

### 1. Archivo de Configuración
PyQuotex buscará automáticamente un archivo `config.ini` en la carpeta `settings`. Si no existe, lo creará solicitando las credenciales:

```ini
[settings]
email=tu_email@gmail.com
password=tu_contraseña
```

### 2. Configuración Directa
Puedes proporcionar las credenciales directamente al crear la instancia del cliente:

```python
client = Quotex(
    email="tu_email@gmail.com",
    password="tu_contraseña"
)
```

## Configuración de SSL/TLS

PyQuotex utiliza un **Contexto SSL Unificado** que soporta **TLS 1.2 y 1.3** con verificación completa de certificados y
suites de cifrado estilo navegador para máxima seguridad y anonimato.

### Cómo funciona internamente

PyQuotex configura automáticamente el contexto SSL:

```python
import ssl
import certifi

# Contexto SSL configurado por la librería
# Contexto SSL unificado utilizado tanto para HTTP como WebSocket
unified_ssl_context = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
unified_ssl_context.minimum_version = ssl.TLSVersion.TLSv1_2
unified_ssl_context.maximum_version = ssl.TLSVersion.TLSv1_3
unified_ssl_context.set_ciphers('ECDHE-ECDSA-AES128-GCM-SHA256:...') # Cifrados estilo navegador
unified_ssl_context.load_verify_locations(certifi.where())
```

**Medidas de seguridad aplicadas:**
- `check_hostname = True` — verificación del hostname del certificado
- `cert_reqs = ssl.CERT_REQUIRED` — certificado obligatorio
- **JA3 Fingerprint** estilo navegador para evitar la detección de bots
- Identidad SSL consistente entre las sesiones HTTP y WebSocket

### Windows
Para Windows, es necesario OpenSSL con soporte TLS 1.3:
1. Descarga el instalador desde [Openssl-Windows](https://slproweb.com/products/Win32OpenSSL.html)
2. Instala siguiendo las instrucciones del instalador

### Linux
```bash
sudo apt update
sudo apt install openssl
```

---

Para más información y soporte, puedes unirte al [grupo de Telegram](https://t.me/+Uzcmc-NZvN4xNTQx) de la comunidad.