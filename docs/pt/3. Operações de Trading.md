...
async def buy_and_check_win():
    ...
    status, buy_info = await client.buy(amount, asset, direction, duration)
    if status:
        # Aguardar o resultado (O cronômetro interno é sincronizado com o servidor Quotex)
        # check_win agora retorna uma tupla: (status, lucro)
        win_status, profit = await client.check_win(buy_info["id"])
        
        if win_status == "win":
            print(f"Ganhou! Lucro: {profit}")
        else:
            print(f"Perdeu. Resultado: {profit}")
```
...
### 6.2 Verificação em tempo real (Event-Driven)
```python
async def check_win(id_number):
    # Retorna uma tupla (win_status, profit)
    # win_status: "win" ou "loss"
    # profit: Valor do lucro/perda
