...
class Settings(Browser):
    ...
    async def get_settings(self) -> dict[str, Any]:
        ...
        response = await self.send_request("GET", f"{self.api.https_url}/api/v1/cabinets/digest")
    ...
    async def set_time_offset(self, time_offset: int) -> dict[str, Any]:
        ...
        response = await self.send_request(
            method="POST", url=f"{self.api.https_url}/api/v1/user/profile/time_offset", json=payload
        )
